...
namespace CitrioN.UI
{
  ...
  public class StepSlider : Slider
  {
    ...
    protected int valueVisualsMultiplier = 1;
    ...
    protected float ConvertFloat(float input, bool toInternal)
    {
      if (toInternal)
      {
        return (input / valueVisualsMultiplier);
      }
      else
      {
        var decimals = MathUtility.GetDecimals(StepSize);

        var value = (valueVisualsMultiplier * input).Truncate(decimals);
        return value;
      }
    }
